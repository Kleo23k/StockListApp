...
import searchStocks, { formSubmission} from "./stocks.js";
...
async function getTimeSeries(filterBy) {
    try {
        const historicalData = await apiService.getTimeSeries(stockSymbol)
...
async function getScores() {
    try {
        ...
        const formatGroupScore = (score) => score?.avg ? score.avg.toFixed(1) : "N/A";
        ...
        const peerEnv = formatGroupScore(peerEnvironmentPerformance);
        const peerGov = formatGroupScore(peerGovernancePerformance);
        const peerSoc = formatGroupScore(peerSocialPerformance);
